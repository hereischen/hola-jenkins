...
#### 3. Configure  `seedJob`  to use  `hola-jenkins`github repository
...
#### 4. Configure  `seedJob`  to use the  `seed.groovy`  file we will store in github
...
#### 5. Configure  `seedJob`  to take a parameter as the job name
...
4.  In the  `Description`  field, enter  `The name of your job`(typo in the image). 
![Job name](/docs/images/DSL/jobname.png)

#### 6. Creating a simple demo Freestyle job
...
#### 8. Approving our `seed.groovy`  script
...
#### 2. Configure default Shared Library set up for Jenkins
...
#### 3. Create `seed`  to use a  `new-imp`  branch we will create in  `hola-jenkins`
